...
if($idPost != 0){
  ...
  if($idPost==1){
    ...
    $sql = "  INSERT INTO `imagenes`( `url`, `estado`, `idProducto`) VALUES (?,1,?);";
...
if($idPost != 0){
  $id = isset( $data['id']) ?  $data['id'] : "-";
  if($idPost == -1){
    $sql = "UPDATE `imagenes` SET estado = 0 WHERE idImagenes = ?";
...
$stmt = $conn->prepare("SELECT * FROM `imagenes` where idProducto = ? and estado = 1 order by 1 desc ");
$stmt->bind_param("i", $id);
